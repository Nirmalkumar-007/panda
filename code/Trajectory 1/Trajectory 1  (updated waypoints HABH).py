...
class PandaCmdSub(Node):
    #ROS2 Node to move robot along trajectory waypoints with Cartesian impedance control and SLERP interpolation
    ...
    def __init__(self, desk: panda_py.Desk, panda: panda_py.Panda):
        ...
        # Waypoints
        self.point_A = self.home_pos + np.array([0.3, -0.5, -0.4])
        self.point_B = self.point_A + np.array([0.0, 0.9, 0.0])
        self.waypoints = [
            self.home_pos,
            self.point_A,
            self.point_B,
            self.point_A,   # go back to point A
            self.home_pos   # finally return home
        ]

        self.orientations = [self.home_quat] * len(self.waypoints)
        self.steps = 100  # number of steps per segment
        # self.steps = 10   *Very rough, jerky
        # self.steps = 30   *Fast, not very smooth
        # self.steps = 60   *Good
    ...
    def run_trajectory(self):
        ...
        for i in range(len(self.waypoints) - 1):
            ...
            self.get_logger().info(f"Reached waypoint {i+1}, pausing 2 seconds")
            time.sleep(2.0)
